...
class WindowStateStore:
    ...
    def update(self, *, save: bool = False, **fields: object) -> WindowState:
        """Меняет поля состояния в памяти.

        По умолчанию на диск **не** пишет: состояние окна сохраняется один
        раз, при закрытии приложения (`save()`), иначе частые записи изнашивают
        SSD. `save=True` — для тех редких случаев, когда запись нужна сразу.
        """
