...
* **Стрелки в full-screen программах.** `PtySession.send_keys()` теперь
  ...
  включила application cursor keys. Наш `TerminalKeymap` по-прежнему жёстко
  шлёт CSI, поэтому `vim`/`htop`/`less` стрелки пока получают неверные — это
  следующий шаг, требует живой проверки в окне.
