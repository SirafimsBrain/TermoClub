...
## Implementation notes, round 8: Flet 1.0.0 и smartcli 0.3.2 — сверка API

Обновление зависимостей (отчёт `after_update_20260917.md`) проверено не
чтением, а прогоном: `flet==1.0.0` + `smartcli-toolkit==0.3.2` поставлены в
отдельное окружение, на них собран проект и прогнан весь набор. Дополнительно
весь `termoclub/` разобран статически: 68 различных имён `ft.*` (0
отсутствующих) и все ключевые аргументы в `ft.X.Y(...)` (0 несовпадений с
полями dataclass'ов и сигнатурами 1.0.0). `ft.run`, `page.run_task`,
`window.close()`, `PageResizeEvent`, `page.on_resize`, async
`Clipboard.get/set`, `FilePicker.pick_files`, `UrlLauncher.launch_url`,
`TextStyle.height`, `Stack.fit` — на месте.

### Что исправлено

* **`show_snack` показывал пустоту.** Во Flet 1.0 у `Page` нет поля
  `snack_bar`, и уведомление — обычный `DialogControl` через
  `page.show_dialog()`. Важная деталь: присваивание `page.snack_bar = ...`
  **не падает** (контролы 1.0 — dataclass'ы без запрета новых атрибутов),
  поэтому ошибка была тихой: сообщение просто не доходило до клиента. Теперь
  уведомление уходит в `show_dialog`, который сам выставляет `dialog.open`.
* **`SegmentedButton.selected` ломал вкладку Plugins.** Во Flet 1.0 поле —
  `list[str]`, и `set` конструктор принимает, но сериализация в msgpack
  падает («can not serialize 'set' object»): кодек особо обрабатывает только
  `list`/`dict`/dataclass. Путь достижим: `plugins.disabled_plugins` —
  `multi_choice` с пустыми вариантами, а режим (сегменты/чекбоксы) выбирается
  по `len(spec.choices) <= 4`, то есть всегда сегменты. Теперь список.
* **Тест диалогов проверял заглушку, а не контракт.** У реального `ft.Page`
  нет атрибута `dialogs`; `_StubPage` заводил его сам. Заглушка переименована
  в `shown_dialogs`, и тест проверяет то, что действительно требует
  `show_dialog`: что контрол является `ft.DialogControl`.
* **Переносимость уровня логирования.** `logging.getLevelNamesMapping()` есть
  только с 3.11: на 3.10 падали 3 теста `SettingsApplier`. Добавлен разбор
  имени через `getLevelName()` для младших версий (README и так требует 3.13+,
  но теперь floor не держится на одном вызове).
* **`SmartCLIScreen.row()`** больше не падает, если апстрим уберёт
  `ScreenModel.screen`: запасной путь через публичный `row_cells()` (без
  курсива и подчёркивания — в `CellAttrs` их нет).
* Гигиена: позиционные `ft.Padding(...)` в `WorkspaceTabBar`/`SessionCardList`
  заменены на `only()`/`symmetric()`; README больше не утверждает, что Flet
  «пришпилен к линии 0.86 и 1.0 не проверен», и команда установки тестовых
  зависимостей дополнена `-r requirements.txt`.

### Что в отчёте оказалось неверным

* P0-1: речь не о падении (`AttributeError`), а о тихом no-op — см. выше.
* P1-2: не «рантайм не падает», а именно падение отрисовки на сериализации;
  пункт следовало помечать P0.
* P1-1 (`page.services.append` в `LinkOpener`/`PathPicker`) — не дефект:
  `page.services` и реестр саморегистрации сервисов (`Page._services`) это
  разные списки, дублирования нет, код работает на обеих версиях. Оставлено
  намеренно: саморегистрация в `Service.init()` подавляет ошибку, когда
  контекста страницы нет, и явная регистрация страхует этот случай.
* `KeyboardEvent` в 1.0.0 не изменился (`key`/`shift`/`ctrl`/`alt`/`meta`) —
  нового канала для кириллицы там не появилось.

### Что даёт обновление

* **Стрелки в full-screen программах.** `PtySession.send_keys()` теперь
  учитывает DECCKM и отдаёт навигацию в SS3 (`ESC O A`), когда программа
  включила application cursor keys. У нас это сделано своим кодом в раунде 9
  (см. ниже) — общий `TerminalKeymap` теперь учитывает режим, поэтому
  `vim`/`htop`/`less`/`mc` получают стрелки в ожидаемой форме.
* **Для кириллицы не даёт ничего** — канал по-прежнему только IME-поле.
* **`PtySession.close()` по-прежнему ~1.03 с** (SIGTERM интерактивный `bash`
  игнорирует): фоновый поток в `terminate()` остаётся нужным.
* Неиспользуемые новые возможности 0.3.2: `snapshot()`, `wait_ready`,
  `content_hash()`.

### Проверка

`252 passed` и на `flet 1.0.0` + `smartcli-toolkit 0.3.2`, и на прежних
`0.86.5` + `0.2.3` — правки не привязаны к одной версии. Headless
подтверждение починки: `SegmentedButton` для `plugins.disabled_plugins`
сериализуется в msgpack без ошибки (было `TypeError`), `show_snack` создаёт
`SnackBar`, зарегистрированный как `DialogControl`. Визуальная приёмка (цвет,
курсор, живой `vim`) по-прежнему возможна только в окне.

## Implementation notes, round 9: серая вкладка Settings, `mc` и два источника размера

Три независимых дефекта из одного прогона: `mc` разъезжался и не слушал
стрелки, вкладка Settings была пустой серой областью, а панель вкладок
показывала созданную, но не нарисованную сессию.

### 1. Серая вкладка Settings — `TextThemeStyle` в `Text.style`

Во Flet `Text.style` — это `TextStyle`, а `theme_style` — `TextThemeStyle`.
Клиент разбирает `style` как map (`packages/flet/lib/src/utils/text.dart`,
`parseTextStyle`: `value["weight"]`, `value["size"]`, ...). Enum
сериализуется строкой (`"titleLarge"`), у `String` в Dart нет `operator []`
— падение на сборке виджета.

Дальше работает Flutter: исключение в `build()` заменяет контрол на
`ErrorWidget`, а `RenderErrorBox` объявлен `sizedByParent` и красит
`#C0C0C0` на всю доступную область (`rendering/error.dart`; в debug там
красный `#900000`, в release — светло-серый). Отсюда и «пустая серая
вкладка»: заголовок «Settings» и заголовок категории закрывали собой всю
панель. Из Python это не видно — Flet-валидация такое значение пропускает,
msgpack сериализуется без ошибок; на клиент ошибка уходит только при
отрисовке.

Исправлено в шести местах (`app/pages/settings.py`, `home.py`, `logs.py`,
`app/ui/settings/SettingsPanel.py`, `EditorSession`, `RdpSession`):
`style=` -> `theme_style=`. Строка `app/ui/theme_style_test.py` статически
проверяет, что шаблон не вернётся: клиентскую ошибку иначе не поймать.

### 2. Вкладка Plugins: `SegmentedButton` без сегментов

`plugins.disabled_plugins` — `multi_choice` с пустым списком вариантов до
первого сканирования плагинов, а режим выбирается по
`len(choices) <= SEGMENT_LIMIT`, то есть всегда сегменты. Со списком `set`
разобрались в раунде 8, но осталось второе состояние: Flet отвергает
контрол без единого сегмента на исходящей валидации
(«segments must contain at least one visible Control»), `ValueError` из
`update()` уходил наверх и ломал категорию. Теперь при пустом списке
вариантов показывается пояснение, а после пересканирования панель
пересобирает строки и режим выбирается заново.

Регрессию стережёт `app/ui/settings/SettingsPanel_test.py`: валидацию и
сериализацию (`app/ui/flet_contract.py`) прогоняют по всему дереву контролов
каждой категории схемы.

### 3. `mc`: стрелки не работали — не было DECCKM

Полноэкранные программы включают application cursor keys (`smkx=\E[?1h\E=`
в terminfo) и берут навигацию из terminfo: у `xterm` это `kcuu1=\EOA`,
`kcub1=\EOD`, `khome=\EOH`. Наш `TerminalKeymap` жёстко шёл в CSI-форме.

Воспроизведено на живом `curses` (та же машинерия, что у `mc`): `ESC O A`
-> `KEY_UP` (259), `ESC [ A` -> `27`. Проверено и после правки, через сам
`TerminalSession`: приложение в PTY включает режим, сессия читает его из
своего же вывода (`CSI ? 1 h/l`, с хвостом между чанками), и стрелка
декодируется как `KEY_UP`.

### 4. `mc` разъезжался: два источника размера перебивали друг друга

Размер приходил двумя путями: событие контейнера терминала (настоящие
пиксели) и запасная оценка по размеру окна для вкладки, которая события
ещё не получала. Оценка неточна (не знает про панель вкладок и границы) и
применялась ещё и на каждой активации вкладки — то есть после настоящего
размера. В логе это видно как скачки при одном и том же окне:
`282x74` -> `285x78` -> `283x74` -> `318x74` -> `285x78`. Каждый скачок
уходит в PTY как `TIOCSWINSZ` + SIGWINCH, полноэкранная программа
перерисовывается под новый размер — отсюда и «разъехавшийся» `mc` с
обрезанным правым краем и нижними строками.

Теперь контейнер — единственный источник: `TerminalView.measured`
включается первым же непустым событием размера, а
`TerminalSession.resize_to_area` после этого не делает ничего. Оценка
осталась только для вкладки, не получившей ни одного события.

### Проверка

`265 passed` на `flet 0.86.5` и на `flet 1.0.0`. Новые тесты: статическая
проверка `style=ft.TextThemeStyle`, контракт клиента по всем категориям
настроек, SS3-таблица в `TerminalKeymap`, отслеживание DECCKM в сессии
(включая разорванную между чанками последовательность), приоритет размера
контейнера над оценкой по окну. Живая приёмка (цвет, курсор, `mc` в окне)
по-прежнему требует GUI.
